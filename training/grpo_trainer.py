# Copied from https://github.com/huggingface/trl/blob/main/trl/trainer/grpo_trainer.py
...
from trl.import_utils import is_liger_kernel_available, is_deepspeed_available, is_rich_available, is_vllm_available
...
class GRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.beta == 0.0:
            ...
            if "qwen2-a" in model.config._name_or_path.lower(): # for Qwen2-Audio
                self.ref_model = Qwen2AudioForConditionalGeneration.from_pretrained("Qwen/Qwen2-Audio-7B-Instruct", **model_init_kwargs)
            elif "omni" in model.config._name_or_path.lower(): # for Qwen2-Omni or Ke-Omni-R
                self.ref_model = Qwen2_5OmniThinkerForConditionalGeneration.from_pretrained(model.config._name_or_path, **model_init_kwargs)
            else:
        ...
        if self.use_vllm:
            ...
            if self.accelerator.is_main_process:
                print(f"Connecting to vLLM server at {args.vllm_server_host}:{args.vllm_server_port}")
    ...
    def _move_model_to_vllm(self):
        ...
        # Helper function to map HF names to vLLM names based on vLLM's internal structure
        # for Qwen2_5OmniThinkerForConditionalGeneration
        def map_hf_name_to_vllm(hf_name, is_peft_model_flag):
            original_hf_name = hf_name # Keep original for PEFT checks if needed

            if is_peft_model_flag:
                # Basic PEFT prefix cleaning - adjust if needed based on specific PEFT setup
                if hf_name.startswith("base_model.model."):
                     hf_name = hf_name.removeprefix("base_model.model.")
                elif hf_name.startswith("base_model."):
                     hf_name = hf_name.removeprefix("base_model.")

                # Handle potential PEFT suffixes or patterns
                hf_name = hf_name.replace(".base_layer", "")
                if "modules_to_save.default." in hf_name:
                    hf_name = hf_name.replace("modules_to_save.default.", "")

                # Skip PEFT specific internal params if they haven't been removed yet
                # (Check based on original name to avoid issues with mapped name)
                if hasattr(self.model, 'prefix') and self.model.prefix in original_hf_name:
                    return None # Skip this weight
                if "original_module" in original_hf_name:
                    return None # Skip this weight


            # Map prefixes to vLLM structure
            # Assuming standard HF prefixes: model., lm_head., vision_tower., audio_tower.
            # Map to vLLM prefixes: language_model.model., language_model.lm_head., visual., audio_tower.
            if hf_name.startswith("model."):
                # Map language model weights
                return "language_model." + hf_name
            elif hf_name.startswith("lm_head."):
                # Map LM head weights
                return "language_model." + hf_name
            elif hf_name.startswith("vision_tower."): # Check actual HF prefix if different
                # Map vision tower weights
                return "visual." + hf_name.removeprefix("vision_tower.")
            elif hf_name.startswith("audio_tower."): # Check actual HF prefix if different
                # Map audio tower weights
                return "audio_tower." + hf_name.removeprefix("audio_tower.")
            elif hf_name.startswith("visual."): # Handle case where HF already uses 'visual.'
                return hf_name
            # Add other potential mappings if needed (e.g., embeddings)
            # elif hf_name.startswith("embed_tokens."):
            #     return "language_model.model." + hf_name
            else:
                # If no specific mapping, it might be an error or a parameter already matching vLLM's structure.
                # Log a warning for unmapped weights.
                # logger.warning(f"Unmapped weight name: {hf_name}. Sending as is.")
                # Return the potentially PEFT-cleaned name; vLLM loader might handle it or error.
                return hf_name
        ...
        if is_peft:
            ...
            with gather_if_zero3(list(self.model.parameters())):
                ...
                for name, param in self.model.named_parameters():
                    vllm_name = map_hf_name_to_vllm(name, is_peft_model_flag=True)

                    if vllm_name is None: # Skip weights filtered by the mapping function
                        continue

                    if self.accelerator.is_main_process:
                        # print(f"DEBUG: Sending PEFT weight: {name} -> {vllm_name}") # Uncomment for debug
                        self.vllm_client.update_named_param(vllm_name, param.data)
            ...
            for name, param in self.model.named_parameters():
                with gather_if_zero3([param]):
                    vllm_name = map_hf_name_to_vllm(name, is_peft_model_flag=False)
                    if vllm_name is None: # Should generally not happen for non-PEFT, but check anyway
                         continue

                    if self.accelerator.is_main_process:
                        # print(f"DEBUG: Sending non-PEFT weight: {name} -> {vllm_name}") # Uncomment for debug
                        self.vllm_client.update_named_param(vllm_name, param.data)
    ...
    def _generate_and_score_completions(
        ...
        sampling_rate = self.processing_class.feature_extractor.sampling_rate            
        if "Qwen2-Audio" in self.model.name_or_path: # for Qwen2-Audio
